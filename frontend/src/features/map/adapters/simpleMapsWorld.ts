...
function findCountryPath(svg: SVGElement, code: string): SVGPathElement | SVGGElement | null {
  ...
  if (byGetEl && svg.contains(byGetEl)) return byGetEl as SVGPathElement | SVGGElement;
...
export class SimpleMapsWorldAdapter implements WorldMapAdapter {
  ...
  private attachClickHandlers(root: HTMLElement): void {
    this.clickAbort?.abort();
    this.clickAbort = new AbortController();
    const { signal } = this.clickAbort;
    ...
    const handleClick = (e: Event): void => {
      ...
      let iso2 =
        ...
        (target.closest?.('[data-state]') as HTMLElement)?.getAttribute?.('data-state') ??
        (target.closest?.('[data-iso2]') as HTMLElement)?.getAttribute?.('data-iso2');
      if (!iso2) {
        const el = target.closest?.('path, g') as HTMLElement | null;
        const id = (el?.getAttribute?.('id') ?? (el as SVGElement & { id?: string })?.id ?? '').trim();
        if (/^[A-Za-z]{2}$/.test(id)) iso2 = id;
      }
      if (iso2) this.handlers.onCountryClick?.(iso2);
    };
    ...
    const handleMouseMove = (e: Event): void => {
      ...
      this.handlers.onCountryHover?.(iso2 ?? null);
    };
    ...
    root.addEventListener('click', handleClick, { capture: true, signal });
    root.addEventListener('mousemove', handleMouseMove, { signal });
    root.addEventListener('mouseleave', handleMouseLeave, { signal });
  }
  ...
  setSelectedCountry(iso2: string | undefined): void {
    ...
    const pid = patternId(iso2);
    const existing = defs.querySelector(`#${pid}`);
    if (existing) existing.remove();

    const flagUrl = getCountryFlagPath(iso2);
